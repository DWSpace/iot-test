<template>
  <div class="flex flex-col flex-1 items-center font-NotoJP">
    ...
    <div v-if="route.query.preview" class="text-white p-4 bg-weather-secondary w-full text-center">
      <p>現在、この都市をプレビューしています。"+" アイコンをクリックして、この都市の追跡を開始してください。</p>
    </div>
    <!-- weather overview -->
    <div class="flex flex-col items-center text-white py-12">
      <h1 class="text-4xl mb-2">{{ route.params.city }}</h1>
      <p class="text-sm mb-12">{{ datetime() }}</p>
      <p class="text-8xl mb-8">{{ Math.round(weatherData.currently.temperature) }}℃</p>
      <img class="w-[150px] h-auto" :src="`/src/assets/img/weather/${weatherData.currently.icon}.png`" alt="" />
    ...
    <div class="max-w-5xl w-full py-12">
      <div class="mx-8 text-white">
        ...
        <div class="flex gap-10 basis-11 overflow-x-scroll">
          <div v-for="hour in weatherData.hourly.data" class="flex flex-col items-center">
            <p class="whitespace-nowrap w-11 text-center text-base">{{
                new Date(hour.time * 1000).toLocaleTimeString([],
                  { hour: 'numeric' })
            }}</p>
            <img class="w-[45px] h-[45px]" :src="`/src/assets/img/weather/${hour.icon}.png`" alt="" />
            <p class="text-right">{{ Math.round(hour.precipProbability * 100) }}%</p>
    ...
    <div class="max-w-5xl w-full py-12 text-white">
      <div class="mx-8">
        ...
        <div v-for="dayName in weatherData.daily.data" class="flex items-center border-b-[1px] border-indigo-500/50">
          <p class="flex-1">
            {{ new Date(dayName.time * 1000).toLocaleString("ja-JP",
                { weekday: 'long' })
            }}
          </p>
          <img class="w-[45px] h-[45px]" :src="`/src/assets/img/weather/${dayName.icon}.png`" alt="" />
          <div class="flex-[2_1_0%]">
            <p class="text-center">高: {{ Math.round(dayName.temperatureMax) }}℃</p>
            <p class="text-center">低: {{ Math.round(dayName.temperatureMin) }}℃</p>
          </div>
          <div class="flex-1">
            <p class="text-center">雨の可能性:</p>
          </div>
          <div class="flex-1">
            <p class="text-right">{{ Math.round(dayName.precipProbability * 100) }}%</p>
          </div>
        </div>
...
const getWeatherData = async () => {
  try {
    const weatherData = await fetch(`https://api.pirateweather.net/forecast/qpkwypu4WKLYa3Ycy9cNBzuUEStELW4Bp2d8Uki0/${route.query.lat},${route.query.lon}?&units=ca&exclude=minutely`)
