<template>
    <div class="flex py-6 px-7 h-20 bg-weather-secondary rounded-md shadow-sm cursor-pointer font-NotoJP">
        <div class="flex flex-col flex-1 self-center">
            <h2 class="text-xl">{{ city.city }}</h2>
        </div>
        <div class="flex flex-col flex-1 gap-2 self-center">
            <span class="text-xl ">
        ...
        <div class="flex flex-col flex-1 gap-2 self-center">
            <span class="text-xl ">
                <img class="w-[50px] h-[50px] object-cover" :src="`/src/assets/img/weather/${city.weather.icon}.png`"
                    alt="" />
            </span>
        </div>

        <div class="flex flex-col gap-2 self-center">
            <span class="text-xl self-end">
                {{ jpLang(city.weather.summary) }}
            </span>
        </div>
    </div>
</template>
