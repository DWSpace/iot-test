<template class="font-NotoJP text-white">
  ...
  <p v-if="savedCities.length == 0">場所が追加されていません。</p>
</template>
<script setup>
import { ref } from "vue";
...
const getCities = async () => {
  if (localStorage.getItem("savedCities")) {
    ...
    savedCities.value.forEach((city) => {
      res.push(
        `https://api.pirateweather.net/forecast/qpkwypu4WKLYa3Ycy9cNBzuUEStELW4Bp2d8Uki0/${city.coords.lat},${city.coords.lon}?&units=ca&exclude=minutely,hourly,daily`
      );
    });
